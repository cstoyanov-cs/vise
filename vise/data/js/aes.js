// Minimal rapydscript compatibility shim.
//
// The aes module body below was generated by rapydscript from client/aes.pyj
// (commit 4497f8d of this repo, blob e9954b5744c42fe52ef202c4cb21648d12e8551d).
// It references rapydscript runtime helpers (_$rapyd$_extends, the kwargs
// protocol, str.format, etc.). We provide the minimum those references need.
//
// The shim is namespaced under `_rapyd$` so it doesn't collide with anything
// in the surrounding module graph. The legacy code references these as
// `_$rapyd$_extends`, etc. (with the $ inside the name).

const _$rapyd$_kwargs_symbol = (typeof Symbol === "function")
    ? Symbol("kwargs-object")
    : "kwargs-object-rapyd-shim";

let _$rapyd$_object_counter = 0;

// Prototype-based inheritance: the legacy code uses
//   _$rapyd$_extends(Child, Parent)
// to wire up prototypes before attaching methods.
function _$rapyd$_extends(child, parent) {
    child.prototype = Object.create(parent.prototype);
    child.prototype.constructor = child;
}

// Rapydscript's deep equality helper. Legacy aes code compares Uint32Array
// tags and Object dicts (e.g. {iv, cipherbytes, tag}) via
// `_$rapyd$_not_equals(a, b)`, which expands to
//     !_$rapyd$_equals(a, b)
// where _equals does:
//   (1) primitive `===`
//   (2) `__eq__` method dispatch on either operand
//   (3) array-like deep compare (length + per-element ===)
//   (4) Object deep compare (keys + per-key ===)
// We implement only the subset that aes.js exercises: primitive === and
// array-like deep compare. The aes module never compares plain Objects here.
function _$rapyd$_equals(a, b) {
    if (a === b) return true;
    if (a && typeof a.__eq__ === 'function') return !!a.__eq__(b);
    if (b && typeof b.__eq__ === 'function') return !!b.__eq__(a);
    const aArr = a && typeof a.length === 'number' && typeof a !== 'string';
    const bArr = b && typeof b.length === 'number' && typeof b !== 'string';
    if (aArr && bArr) {
...
function _$rapyd$_not_equals(a, b) { return !_$rapyd$_equals(a, b); }

// Coercion used in `with` statement suppress logic.
const _$rapyd$_bool = (v) => !!v;

// The legacy code expects `_$rapyd$_modules.aes.<name> = <name>` to expose
// each top-level symbol. The IIFE writes into this object; we read from it
// at the bottom to populate the module's exports.
const _$rapyd$_modules = { aes: {} };

// Python-style string formatting. Only used inside as_hex() (debug helper)
// and bytes_to_string_slow() (TextDecoder fallback). Subset implemented here:
//   str.format(template, ...args)   - replaces {0}, {1}, ...
//   str.rstrip(s, chars)            - strips trailing chars
const str = {
    format(template, ...args) {
        let i = 0;
        return template.replace(/\{[0-9]+\}/g, () => String(args[i++]));
    },
    rstrip(s, chars) {
        // chars is a string of class-of-chars (e.g. "\u0000"); build a class.
        const escaped = chars.replace(/[-\\]/g, "\\$&");
        return s.replace(new RegExp("[" + escaped + "]+$"), "");
    },
};

// Rapyd list helpers. `_$rapyd$_list_decorate` accepts an array and wraps it
// in a "list-like" object (rapydscript's PyList); for plain Uint8Arrays and
// Arrays we can pass through. `_$rapyd$_list_constructor` is rapydscript's
// factory for a fresh PyList; Array works fine.
function _$rapyd$_list_decorate(arr) { return arr; }
function _$rapyd$_list_constructor(arr) { return Array.from(arr); }

// Rapydscript's iter helper wraps both array-like and iterable values into
// an indexable sequence. Only called from as_hex() (debug), which is unused
// in our hot path. A thin wrapper around Array.from is enough.
function _$rapyd$_Iterable(it) {
    if (it && typeof it.length === "number") return it;
    return Array.from(it);
}

// `_$rapyd$_chain_assign_temp` is rapydscript's way to share a value across
// several variables. The legacy code declares it locally with `var` and uses
// it as a scratch variable; defining it once at module scope is equivalent.
let _$rapyd$_chain_assign_temp;

// rapydscript throws ``new ValueError(...)`` for invalid inputs. Define a
// minimal Error subclass that carries the right ``name``.
...
// Pull each public symbol out of the IIFE-attached namespace and re-export.
// crypto.js only needs GCM (and friends AES / ModeOfOperation / GaloisField
// are exposed for completeness so future modules can compose new modes
// without re-extracting from the legacy blob).
const AES = _$rapyd$_modules.aes.AES;
const GCM = _$rapyd$_modules.aes.GCM;
const CTR = _$rapyd$_modules.aes.CTR;
const CBC = _$rapyd$_modules.aes.CBC;
const ModeOfOperation = _$rapyd$_modules.aes.ModeOfOperation;
const GaloisField = _$rapyd$_modules.aes.GaloisField;
const string_to_bytes = _$rapyd$_modules.aes.string_to_bytes;
const bytes_to_string = _$rapyd$_modules.aes.bytes_to_string;
const random_bytes_secure = _$rapyd$_modules.aes.random_bytes_secure;
const random_bytes_insecure = _$rapyd$_modules.aes.random_bytes_insecure;
